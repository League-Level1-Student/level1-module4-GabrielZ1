...
public class PigLatinTranslator implements ActionListener {
	...
	JButton translateToPigLatinButton = new JButton();
	JButton translateToEnglishButton = new JButton();
	JTextField pigLatin = new JTextField();
	JButton speakPigLatinButton = new JButton();

	public static void main(String[] args) {
		PigLatinTranslator pigLatinTranslator = new PigLatinTranslator();	
		pigLatinTranslator.run();
	}
	...
	public void run() {
		...
		translateToPigLatinButton.setText(">>");
		translateToEnglishButton.setText("<<");

		translateToPigLatinButton.addActionListener(this);
		translateToEnglishButton.addActionListener(this);

		pigLatin.setColumns(20);

		speakPigLatinButton.setText("Speak");

		speakPigLatinButton.addActionListener(this);
		
		frame.setLayout(new FlowLayout());
		frame.add(english);
		frame.add(translateToPigLatinButton);
		frame.add(translateToEnglishButton);
		frame.add(pigLatin);
		frame.add(speakPigLatinButton);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(translateToPigLatinButton == buttonPressed) {
			pigLatin.setText(translateEnglishToPigLatin(english.getText()));

		}
		if(translateToEnglishButton == buttonPressed) {
			english.setText(translatePigLatinToEnglish(pigLatin.getText()));

		}
		if(speakPigLatinButton == buttonPressed) {
